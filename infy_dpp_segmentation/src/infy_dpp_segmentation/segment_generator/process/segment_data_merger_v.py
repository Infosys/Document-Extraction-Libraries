...
class SegmentDataMerger:
    ...
    def segment_data_merge(self, merge_requirements) -> list:
        if len(self.segment_data_list_of_list)==1:
                if merge_requirements.get('enabled'):
                    vertical_max_gap = merge_requirements.get('vertical_adjacent_segments_max_gap_in_pixel')
                    horizontal_max_gap = merge_requirements.get('horizontal_adjacent_segments_max_gap_in_pixel')
                    group_adjacent_boxes = self.group_adjacent_boxes(self.segment_data_list_of_list[0], vertical_max_gap, horizontal_max_gap)
        ...
        if merge_requirements.get('enabled'):
            vertical_max_gap = merge_requirements.get('vertical_adjacent_segments_max_gap_in_pixel')
            horizontal_max_gap = merge_requirements.get('horizontal_adjacent_segments_max_gap_in_pixel')
            group_adjacent_boxes = self.group_adjacent_boxes(self.segment_data_result_list, vertical_max_gap, horizontal_max_gap)
            return group_adjacent_boxes

        return self.segment_data_result_list
    
    def group_adjacent_boxes(self, merged_list, vertical_max_gap, horizontal_max_gap):
        # Sort the list by 'page', 'y1', and 'x1'
        sorted_list = sorted(merged_list, key=lambda data: (data['page'], data['content_bbox'][1], data['content_bbox'][0]))
        ...
        for data in sorted_list:
            ...
            for group in grouped_boxes:
                ...
                if any(box['page'] == data['page'] and self.is_adjacent(box['content_bbox'], data['content_bbox'], vertical_max_gap, horizontal_max_gap) for box in group):
        ...
        return self.merge_boxes(grouped_boxes)
    ...
    def segment_data_insert(self, data):
        ...
        if overlapping_indices and self.prefer_larger_segments:
            ...
            new_data_area = (data['content_bbox'][2] - data['content_bbox'][0]) * (data['content_bbox'][3] - data['content_bbox'][1])
    ...
    def is_adjacent(self, bbox1, bbox2, threshold_tb, threshold_rl):
        # Check if the boxes are vertically adjacent
        vertical = abs(bbox1[3] - bbox2[1]) <= threshold_tb and (max(bbox1[0], bbox2[0]) <= min(bbox1[2], bbox2[2]))
        # Check if the boxes are horizontally adjacent
        horizontal = abs(bbox1[2] - bbox2[0]) <= threshold_rl and (max(bbox1[1], bbox2[1]) <= min(bbox1[3], bbox2[3]))
        # If either condition is met, the boxes are adjacent
        return vertical or horizontal
     
    def merge_boxes(self, grouped_boxes):
        # Merge all bounding boxes in each group into a single larger bounding box
        merged_boxes = []
        for group in grouped_boxes:
            # Sort each group by 'y1' and 'x1'
            group.sort(key=lambda data: (data['content_bbox'][1], data['content_bbox'][0]))
