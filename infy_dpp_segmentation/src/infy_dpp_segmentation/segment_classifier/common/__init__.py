    ...
    def get_rule_class_instance(cls, rc_module_name: str, rc_entity_name: str = None):
        ...
        if rc_entity_name:
            rule_mudule = import_module(
                f"infy_dpp_segmentation.segment_classifier.rules.{rc_entity_name}.{rc_module_name}")
        else:
            rule_mudule = import_module(f"infy_dpp_segmentation.segment_classifier.rules.{rc_module_name}")
