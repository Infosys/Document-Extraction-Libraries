...
class RuleSegmentZigZag(RuleSegmentBaseClass):
    ...
    def generate_sequence_no(self,segment_data_list: list) -> list:
        ...
        _ = [i for i in sorted(enumerate(points_sum), key=lambda x:(x[1][3],x[1][1][0],x[1][2],x[1][1][1]))]
        sorted_list = [i for i in sorted(enumerate(points), key=lambda x:(x[1][3],x[1][1][0],x[1][1][1]))]
        ...
        col_group_sorted_list = [i for i in sorted(new_column, key=lambda x:(x[0],x[1][1][1][1]))]
        sequence_counter = 1
        for idx,data in enumerate(col_group_sorted_list):
            ...
            sequence_counter += 1        
        updated_segment_data_list.sort(key=lambda x: (x['page'],x['sequence']), reverse=False)
