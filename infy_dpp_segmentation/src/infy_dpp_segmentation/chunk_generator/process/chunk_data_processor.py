...
from infy_dpp_segmentation.chunk_generator.process.chunk_data_parser import ChunkDataParser
import infy_dpp_sdk
...
class ChunkDataProcessor:
    ...
    def parse_chunk_data(self, config_file_data, document_data_path_list):
        processor_response_data = infy_dpp_sdk.data.ProcessorResponseData()
        segment_generator_obj = ChunkDataParser(processor_response_data)
        for document_data_path in document_data_path_list:
            org_document_data_json = json.loads(
                obj_store_instance.read_file(document_data_path))
            ...
            processor_response = segment_generator_obj.do_execute(
                document_data=document_data, context_data=org_context_data, config_data=config_file_data)
