...
def test_anchor_text_extraction():
    ...
    assert response_data_list[0].context_data.get('content_extractor').get(
        'image_ocr').get('image_ocr_file_path') is not None
    assert response_data_list[0].context_data.get('content_extractor').get(
        'image_ocr').get('image_file_path') is not None
    assert response_data_list[0].context_data.get('anchor_text_extractor').get(
        'anchor_region_text')[0] == 'SDCKL00562710'
