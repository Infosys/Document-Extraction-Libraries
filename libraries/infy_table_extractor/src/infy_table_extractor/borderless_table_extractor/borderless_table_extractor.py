...
class BorderlessTableExtractor(ite.interface.ITableExtractor):
    ...
    def __init__(self, temp_file_dir: str,
        ...
        Example:
            ...
            >>> from infy_table_extractor.borderless_table_extractor.providers.tesseract_data_service_provider import TesseractDataServiceProvider
            >>> temp_dir = "./data/temp"
            >>> tesseract_path = os.getenv('TESSERACT_PATH')
            >>> token_rows_cols_provider = TesseractDataServiceProvider(tesseract_path)
            >>> token_detection_provider = TesseractDataServiceProvider(tesseract_path)
            >>> extractor = BorderlessTableExtractor(temp_dir, token_rows_cols_provider, token_detection_provider)
            >>> isinstance(extractor, BorderlessTableExtractor)
            True
    ...
    def set_token_enchance_provider(self, token_enhance_provider: DataServiceProviderInterface):
        ...
        Example:
            ...
            >>> from infy_table_extractor.borderless_table_extractor.providers.tesseract_data_service_provider import TesseractDataServiceProvider
            >>> tesseract_path = os.getenv('TESSERACT_PATH')
            >>> token_enhance_provider = TesseractDataServiceProvider(tesseract_path)
            >>> extractor = BorderlessTableExtractor("./temp", token_enhance_provider, token_enhance_provider)
            >>> extractor.set_token_enchance_provider(token_enhance_provider)
            >>> extractor.token_enhance_provider == token_enhance_provider
            True
            >>> logging.disable(logging.NOTSET)
        """
        self.token_enhance_provider = token_enhance_provider
    ...
    def extract_all_fields(self, image_file_path: str, file_data_list: [ite.interface.FILE_DATA] = None,
        ...
        Example:
            ...
            >>> result = extractor.extract_all_fields(input_img, file_data_list=[{"path": input_pdf, "pages": [1]}], config_param_dict={"ocr_tool_settings": {'tesseract': {'psm': '12'}}})
