...
class InfyOcrEngineDataServiceProvider(DataServiceProviderInterface):
    ...
    def __init__(self, infy_ocr_engine_jar_home: str, model_dir_path: str, logger: logging.Logger = None, log_level: int = None):
        ...
        Example:
            ...
            >>> provider = InfyOcrEngineDataServiceProvider(r"C:/MyProgramFiles/InfyOcrEngine", r"C:/MyProgramFiles/AI/models/tessdata")
            >>> isinstance(provider, InfyOcrEngineDataServiceProvider)
            True
    ...
    def get_tokens(self, token_type_value: int, img: np.array,
        ...
        Example:
            ...
            >>> result = provider.get_tokens(1, img_path)
    ...
    def get_text(self, img: np.array, img_cell_bbox_list: [IMG_CELL_BBOX],
        ...
        Example:
            ...
            >>> provider = InfyOcrEngineDataServiceProvider(r"C:/MyProgramFiles/InfyOcrEngine", r"C:/MyProgramFiles/AI/models/tessdata")
            >>> img_path = './data/sample_1.png'  
            >>> img_cell_bbox_list = [{'cell_bbox': [10, 10, 50, 50]}]
            >>> additional_info = {'cell_info': [{'cell_img_path': './data/sample_1.png', 'cell_bbox': [10, 10, 50, 50]}]}
            >>> result = provider.get_text(img_path, img_cell_bbox_list, additional_info=additional_info, temp_folderpath='./data/temp')
            >>> isinstance(result, list)
            True
            >>> logging.disable(logging.NOTSET)
        """
        ...
        image_dict_list_json_file_path = f'{temp_folderpath}/img_list-{str(time.time())}.json'
        ...
        for i, d in enumerate(data):
            result.append(
                {'cell_text': self.__remove_unicode(d.get('data').strip()), 'cell_bbox': additional_info['cell_info'][i]['cell_bbox']})
            if d.get('data') == '':
        ...
        if len(blank_text_index) > 0:
            psm = '6'
            image_dict_list_json_file_path = f'{temp_folderpath}/img_list-{str(time.time())}_{psm}.json'
            with open(image_dict_list_json_file_path, 'w', encoding='utf-8') as json_file:
                json.dump(image_dict_list, json_file, indent=4)

            # ocr extraction with psm 6
            output_json_file, _ = self._call_exe(
                image_dict_list_json_file_path, 'txt', psm)
            output_json_file_path = output_json_file.split('\n', maxsplit=1)[0]
            ...
            for i, d in enumerate(data):
                if d.get('data') != '':
                    result[blank_text_index[i]]['cell_text'] = self.__remove_unicode(d.get(
                        'data').strip())
                if d.get('data') == '':
        ...
        if len(blank_text_index_2) > 0:
            ...
            for i, d in enumerate(data):
                if d.get('data') != '':
                    result[blank_text_index_2[i]]['cell_text'] = self.__remove_unicode(d.get(
                        'data').strip())
        # for data_dict in additional_info['cell_info']:
