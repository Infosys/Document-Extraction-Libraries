...
class BorderedTableExtractor(ite.interface.ITableExtractor):
    ...
    Methods:
        extract_all_fields(image_file_path: str, file_data_list: [ite.interface.FILE_DATA] = None,
                           within_bbox: list = None, config_param_dict: ite.interface.CONFIG_PARAM_DICT = None):
            Extracts table information from an image and returns as a json output and saves it as an excel file, if required.
    """
    ...
    def __init__(self, table_detection_provider: DataServiceProviderInterface,
        ...
        Examples:
            ...
            >>> from infy_table_extractor.bordered_table_extractor.providers import TesseractDataServiceProvider
            >>> tesseract_path = os.getenv('TESSERACT_PATH')
            >>> table_detection_provider = TesseractDataServiceProvider(tesseract_path)
            >>> cell_extraction_provider = TesseractDataServiceProvider(tesseract_path)
            >>> temp_folderpath = "./data/temp"
            >>> extractor = BorderedTableExtractor(table_detection_provider, cell_extraction_provider, temp_folderpath)
    ...
    def extract_all_fields(self, image_file_path: str, file_data_list: [ite.interface.FILE_DATA] = None,
        ...
        Examples:
            ...
            >>> result = extractor.extract_all_fields(image_file_path, within_bbox=[73, 2001, 4009, 937])
