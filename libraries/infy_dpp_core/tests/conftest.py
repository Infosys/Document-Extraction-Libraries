...
@pytest.fixture(scope="module")
def copy_folders_to_root_folder() -> []:
    """Create root folder for testing"""
    return __copy_folders_to_root_folder
...
def __copy_folders_to_root_folder(folders_to_copy: []):
    for folder_to_copy in folders_to_copy:
        if os.path.exists(folder_to_copy[1]):
            shutil.rmtree(folder_to_copy[1])
        shutil.copytree(folder_to_copy[0], folder_to_copy[1])
