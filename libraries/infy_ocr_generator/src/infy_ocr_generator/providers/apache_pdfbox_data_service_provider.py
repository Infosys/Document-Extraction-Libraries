...
from infy_ocr_generator._internal.file_util import FileUtil
...
class ApachePdfboxDataServiceProvider(DataServiceProviderInterface):
    ...
    def __init__(self, config_params_dict: CONFIG_PARAMS_DICT,
        ...
        Examples:
            ...
            ...        }
            ...    }
            >>> pdfbox_ocr_provider = ApachePdfboxDataServiceProvider(config_params_dict=CONFIG_PARAMS_DICT)
            >>> ocr_generator_obj = OcrGenerator(data_service_provider=pdfbox_ocr_provider)
    ...
    def submit_request(self, doc_data_list: [DOC_DATA]) -> list:
        ...
        Examples:
            ...
            >>> pdfbox_ocr_provider = ApachePdfboxDataServiceProvider(config_params_dict=CONFIG_PARAMS_DICT)
            >>> ocr_generator_obj = OcrGenerator(data_service_provider=pdfbox_ocr_provider)
            >>> pdfbox_ocr_provider.submit_request(['./data/sample/1.jpg', './data/sample/swnlp.pdf'])
    ...
    def receive_response(self, submit_req_response_list: list, rerun_unsucceeded_mode: bool = False) -> list:
        ...
        Examples:
            ...
            ... }
            >>> pdfbox_ocr_provider = ApachePdfboxDataServiceProvider(config_params_dict=CONFIG_PARAMS_DICT)
            >>> pdfbox_ocr_provider.receive_response([])
            Traceback (most recent call last):
                ...
            NotImplementedError
            >>> logging.disable(logging.NOTSET)
    ...
    def generate(self, doc_data_list: [DOC_DATA] = None, api_response_list: list = None) -> list:
        ...
        Examples:
            ...
            ... }
            >>> pdfbox_ocr_provider = ApachePdfboxDataServiceProvider(config_params_dict=CONFIG_PARAMS_DICT)
            >>> pdfbox_ocr_provider.generate(['./data/sample/swnlp.pdf'])
            []
            >>> logging.disable(logging.NOTSET)
        """
    ...
    def rescale_dimension(self, doc_ocr_file_path, rescale_data_list: [RESCALE_DATA]) -> list:
        """Rescales the dimensions of the OCR data.

        Args:
            doc_ocr_file_path (str): Path to the OCR file.
            rescale_data_list ([RESCALE_DATA]): List of rescale data.

        Returns:
            list: List of rescaled data.
        Examples:
        >>> import logging
        >>> logging.disable(logging.CRITICAL)
        >>> from infy_ocr_generator.ocr_generator import OcrGenerator
        >>> CONTAINER_ROOT_PATH = "C:/temp/unittest/infy_ocr_generator/test"
        ...
        >>> if os.path.exists(pdf_doc_file_path):
        ...     doc_data_list = [{'doc_path': pdf_doc_file_path}]
        ...     ocr_list = pdfbox_ocr_provider.generate(doc_data_list=doc_data_list)
        ...     if ocr_list:
        ...
        ...             'doc_page_width': 0,
        ...             'doc_page_height': 0,
        ...             'doc_file_path': f'{DATA_ROOT_PATH}/1.jpg',
        ...             'doc_file_extension': 'jpg'
        ...         }]
        ...         page_ocr_list = pdfbox_ocr_provider.rescale_dimension(ocr_file_path, rescale_data)
        ...         assert os.path.basename(page_ocr_list[0]['output_doc']) == "1.jpg_pdfbox.json"
        >>> logging.disable(logging.NOTSET)
        """
        rescaled_data_list = []
