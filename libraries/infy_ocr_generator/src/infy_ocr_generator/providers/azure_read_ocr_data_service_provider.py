...
class AzureReadOcrDataServiceProvider(DataServiceProviderInterface):
    ...
    def __init__(self, config_params_dict: CONFIG_PARAMS_DICT,
        ...
        Example:
            ...
            ...       }
            ...   }
            >>> provider = AzureReadOcrDataServiceProvider(config_params_dict=CONFIG_PARAMS_DICT)
    ...
    def generate(self, doc_data_list: [DOC_DATA] = None, api_response_list: list = None) -> list:
        ...
        Example:
            ...
            ...   }
            >>> provider = AzureReadOcrDataServiceProvider(config_params_dict=CONFIG_PARAMS_DICT)
            >>> api_response_list = [{'input_doc': './data/sample_1.png', 'submit_api': {'response': {'status_code': 200}}, 'receive_api': {'response': {'status': 'succeeded'}}}]
            >>> provider.generate(api_response_list=api_response_list)  
            [{'input_doc': './data/sample_1.png', 'output_doc': './data/sample_1.png_azure_read.json', 'error': ''}]
            >>> logging.disable(logging.NOTSET)
        """
        ocr_list = []
