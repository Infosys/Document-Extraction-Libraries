...
OUTPUT_FILE_SUFFIX = '_azure_form_recognizer.json'


class AzureFormRecognizerOcrDataServiceProvider(DataServiceProviderInterface):
    """Implementation of DataServiceProvider for AZURE Form Recognizer API"""
    ...
    def __init__(self, config_params_dict: CONFIG_PARAMS_DICT,
                 ...
                 log_level: int = None):
        """Creates an instance of Azure Form Recognizer Ocr Data Service Provider
        ...
        Example:
            ...
            ...        }
            ...    }
            >>> provider = AzureFormRecognizerOcrDataServiceProvider(config_params_dict=CONFIG_PARAMS_DICT)
            >>> logging.disable(logging.NOTSET)
        """
        super(AzureFormRecognizerOcrDataServiceProvider,
              self).__init__(logger, log_level)
        self.config_params_dict = self.get_updated_config_dict(
            config_params_dict['azure_form_recognizer'], CONFIG_PARAMS_DICT['azure_form_recognizer'])
    ...
    def submit_request(self, doc_data_list: [DOC_DATA]):
        ...
        Examples:
            ...
            >>> response_list = provider.submit_request(doc_data_list)
            >>> logging.disable(logging.NOTSET)
        """
        def _call_azure_form_recognizer_post_api(in_doc_obj) -> dict:
            ...
            _api_url = self.config_params_dict['endpoint']
            _sub_key = self.config_params_dict['key']
            ...
            try:
                ...
                self.set_api_log(_api_url)
                document_analysis_client = DocumentAnalysisClient(
                    endpoint=_api_url, credential=AzureKeyCredential(_sub_key))
                poller = document_analysis_client.begin_analyze_document(
                    "prebuilt-document", document=input_doc_data)
                response = poller.result().to_dict()
                if len(response) > 0:
                    status = 200
                else:
                    status = 202
        ...
        with concurrent.futures.ThreadPoolExecutor(
                ...
                thread_name_prefix="th_azure_form_recognizer_submit") as executor:
            thread_pool_dict = {
                executor.submit(
                    _call_azure_form_recognizer_post_api,
                    in_doc_obj
    ...
    def receive_response(self, submit_req_response_list: list, rerun_unsucceeded_mode: bool = False) -> list:
        """API of Azure Form Recognizer ocr data service to Received response of asynchorouns call submit request.
        ...
        Examples:
            ...
            ... }
            >>> provider = AzureFormRecognizerOcrDataServiceProvider(config_params_dict=CONFIG_PARAMS_DICT)
            >>> submit_req_response_list = [{'input_doc': './data/sample_1.png', 'submit_api': {'response': {'status_code': 202}}}]
            >>> response_list = provider.receive_response(submit_req_response_list)
            >>> logging.disable(logging.NOTSET)
        """
        def _call_azure_form_recognizer_api(sub_req_res) -> dict:
            try:
        ...
        with concurrent.futures.ThreadPoolExecutor(
                ...
                thread_name_prefix="th_azure_form_recognizer_receive") as executor:
            thread_pool_dict = {
                executor.submit(
                    _call_azure_form_recognizer_api,
                    sub_req_res
    ...
    def generate(self, doc_data_list: [DOC_DATA] = None, api_response_list: list = None) -> list:
        ...
        Examples:
            ...
            ... }
            >>> provider = AzureFormRecognizerOcrDataServiceProvider(config_params_dict=CONFIG_PARAMS_DICT)
            >>> api_response_list = [{'input_doc': './data/sample_1.png', 'submit_api': {'response': {'status_code': 202}}, 'receive_api': {'response': {}}}]
            >>> ocr_list = provider.generate(api_response_list=api_response_list)
            >>> logging.disable(logging.NOTSET)
        """
        ocr_list = []
