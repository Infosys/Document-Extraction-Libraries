...
def pre_test(create_root_folders, copy_files_to_root_folder):
    ...
    FILES_TO_COPY = [
        ['AR_2021-22_page-20-21.pdf', f"{SAMPLE_ROOT_PATH}/input",
        ...
        ['parallel_pipeline_input_config_data.json', f"{SAMPLE_ROOT_PATH}/config",
         f"{STORAGE_ROOT_PATH}/data/config"],
        ['dpp_tf_pipeline_input_config_data_separate_table_content_extractor.json', f"{SAMPLE_ROOT_PATH}/config",
         f"{STORAGE_ROOT_PATH}/data/config"]
...
def test_dpp_segmentation_1():
    ...
    assert len(response_data_list[0].context_data) >= 6
...
def test_dpp_segmentation_parallel_segment_genrators():
    """
        Test case for TF pipeline with parallel_segment_genrators
    """
...
def test_dpp_segmentation_parallel_segment_genrators_with_separate_table_content_extractor_node():
    """
        Test case for TF pipeline with separate table content extractor node in sequence to parallel_segment_genrators
    """
    PROCESSOR_INPUT_CONFIG_PATH = '/data/config/dpp_tf_pipeline_input_config_data_separate_table_content_extractor.json'
    dpp_orchestrator = infy_dpp_sdk.orchestrator.OrchestratorNative(
        input_config_file_path=PROCESSOR_INPUT_CONFIG_PATH)
    response_data_list = dpp_orchestrator.run_batch()
