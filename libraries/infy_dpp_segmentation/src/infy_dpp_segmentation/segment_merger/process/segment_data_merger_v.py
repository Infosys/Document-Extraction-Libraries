...
class SegmentDataMerger:
    ...
    def __group_adjacent_boxes(self, merged_list, vertical_max_gap, horizontal_max_gap, length_threshold=1.7):
            ...
            for group in grouped_boxes:
                for box in group:
                    ...
                    if box not in flagged_segments:
                        same_page = box['page'] == data['page']
                        same_content_type = box['content_type'] == data['content_type']
                        length_check = length <= length_threshold * \
                            self.__length(box['content_bbox'])
                        is_adjacent = self.__is_adjacent(
                            box['content_bbox'], data['content_bbox'], vertical_max_gap, horizontal_max_gap)
                        is_overlapping = self.__overlaps(
                            box['content_bbox'], data['content_bbox'])

                        if same_page and (same_content_type or length_check) and (is_adjacent or is_overlapping):
                            group.append(data)
                            found_group = True
                            break
        ...
        for i, group in enumerate(grouped_boxes):
            ...
            if page not in page_groups:
                page_groups[page] = {}
            page_groups[page][f"Group {i + 1}"] = [data['content_bbox']
                                                   for data in group]
        groups_variable = page_groups
    ...
    def __is_adjacent(self, bbox1, bbox2, threshold_tb, threshold_rl):
        ...
        vertical_distance = abs(bbox1[3] - bbox2[1])
        # Calculate the horizontal distance
        horizontal_distance = abs(bbox1[2] - bbox2[0])

        # Check if the boxes are vertically adjacent
        vertical = abs(bbox1[3] - bbox2[1]) <= threshold_tb and (
            max(bbox1[0], bbox2[0]) <= min(bbox1[2], bbox2[2]) or
            abs((bbox1[0] + bbox1[2]) / 2 - (bbox2[0] + bbox2[2]) / 2) <= 0.5 * (bbox1[2] - bbox1[0]))
