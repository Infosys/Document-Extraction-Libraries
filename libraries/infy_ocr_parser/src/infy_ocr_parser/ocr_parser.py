...
import cv2
from infy_ocr_parser._internal import ocr_helper
from infy_ocr_parser._internal.response import Response
from infy_ocr_parser._internal.ocr_const import RegLabel, PlotBbox
from infy_ocr_parser._internal.ocr_validator import OcrValidator
from infy_ocr_parser._internal.file_util import FileUtil
...
class OcrType(enum.Enum):
    """Enum representing different OCR types.

    Attributes:
        TESSERACT (int): Represents Tesseract OCR.
        ABBYY (int): Represents ABBYY OCR.
    """
    TESSERACT = 1
    ABBYY = 2
...
class OcrParser:
    ...
    This class offers methods to parse OCR files, extract tokens, save tokens as JSON, 
    get bounding boxes for regions, calculate scaling factors, find nearby tokens, 
    and plot bounding boxes on images.

    Attributes:
        ocr_file_list (list): List of OCR documents with full path.
        data_service_provider (DataServiceProviderInterface): Provider to parse OCR file of a specified OCR tool.
        config_params_dict (dict): Configuration dictionary for anchor text match method.
    ...
    def get_tokens_from_ocr(self, token_type_value: int, within_bbox=[],
        ...
        Example:
            ...
            >>> ocr_parser = OcrParser(["./data/sample_1.png.hocr"], data_service_provider)
            >>> tokens = ocr_parser.get_tokens_from_ocr(1)
    ...
    def save_tokens_as_json(self, out_file, token_type_value: int, pages=[], scaling_factor=SCALING_FACTOR) -> dict:
        ...
        Example:
            ...
            >>> data_service_provider = TesseractOcrDataServiceProvider()
            >>> ocr_parser = OcrParser(["./data/sample_1.png.hocr"], data_service_provider)
            >>> result = ocr_parser.save_tokens_as_json("./data/output.json", 1)
    ...
    def get_bbox_for(self, region_definition=[REG_DEF_DICT],
        ...
        Example:
            ...
            >>> data_service_provider = TesseractOcrDataServiceProvider()
            >>> ocr_parser = OcrParser(["./data/sample_1.png.hocr"], data_service_provider)
            >>> bbox = ocr_parser.get_bbox_for([REG_DEF_DICT])
    ...
    def calculate_scaling_factor(self, image_width: int = 0, image_height: int = 0, page: int = 0) -> dict:
        ...
        Example:
            ...
            >>> data_service_provider = TesseractOcrDataServiceProvider()
            >>> ocr_parser = OcrParser(["./data/sample_1.png.hocr"], data_service_provider)
            >>> scaling_factor = ocr_parser.calculate_scaling_factor(1000, 1000, 1)
            >>> isinstance(scaling_factor, dict)
            True
            >>> logging.disable(logging.NOTSET)
        """
    ...
    def get_nearby_tokens(self,
        ...
        Example:
            ...
            >>> data_service_provider = TesseractOcrDataServiceProvider()
            >>> ocr_parser = OcrParser(["./data/sample_1.png.hocr"], data_service_provider)
            >>> nearby_tokens = ocr_parser.get_nearby_tokens(ANCHOR_TXT_DICT)
            >>> isinstance(nearby_tokens,dict)
            True
            >>> logging.disable(logging.NOTSET)
        """
        try:
    ...
    def plot(self, plot_data: PLOT_DATA) -> list:
        ...
        Args:
            plot_data (PLOT_DATA): Data required for plotting.

        Returns:
            list: List of dictionaries containing image page number, original image file path, and output image file path.
        ...
        Example:
            ...
            ...     'images': [{
            ...         'image_page_num': 1,
            ...         'image_file_path': './data/sample_1.png'
            ...     }]
            ... }
            >>> plot_result = ocr_parser.plot(plot_data)
            >>> isinstance(plot_result, list)
            True
            >>> logging.disable(logging.NOTSET)
        """
        response_list = []
    ...
    def __draw_bbox(self, image_file_path: str, tokens: list):
        ...
        Args:
            ...
            tokens (list): List of tokens to draw bounding boxes around.

        Returns:
            str: Path to the image file with bounding boxes drawn.
        """
