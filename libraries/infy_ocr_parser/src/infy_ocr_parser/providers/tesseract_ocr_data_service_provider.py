...
class TesseractOcrDataServiceProvider(DataServiceProviderInterface):
    ...
    def __init__(self, logger: logging.Logger = None,
        ...
        Example:
            ...
            >>> provider = TesseractOcrDataServiceProvider(logger=logger)
        """
    ...
    def init_provider_inputs(self, doc_list: list):
        ...
        Example:
            ...
            >>> logger.addHandler(logging.NullHandler())
            >>> provider = TesseractOcrDataServiceProvider(logger=logger)
            >>> provider.init_provider_inputs(['./data/sample_1.png.hocr'])
            >>> len(provider._ocr_file_data_list) > 0
            True
        """
        if doc_list:
            self._ocr_file_data_list = []
    ...
    def get_line_dict_from(self, pages: list = None,
        ...
        Example:
            ...
            >>> provider = TesseractOcrDataServiceProvider(logger=logger)
            >>> provider.init_provider_inputs(['./data/sample_1.png.hocr'])
            >>> lines = provider.get_line_dict_from(pages=[1])
            >>> len(lines) > 0
            True
            >>> lines[0]['text']
            'GRADE SHEET'
    ...
    def get_word_dict_from(self, pages: list = None,
        ...
        Args:
            pages (list, optional): Page to filter from given `doc_list`. Defaults to None.
            word_dict_list (list, optional): Existing word dictionary to filter certain page(s). Defaults to None.
        ...
        Returns:
            list: List of word dictionary containing the text, words and respective bbox values.
        ...
        Example:
            ...
            >>> provider = TesseractOcrDataServiceProvider(logger=logger)
            >>> provider.init_provider_inputs(['./data/sample_1.png.hocr'])
            >>> words = provider.get_word_dict_from(pages=[1])
            >>> len(words) > 0
            True
            >>> words[0]['text']
            'GRADE'
    ...
    def get_page_bbox_dict(self) -> list:
        ...
        Example:
            ...
            >>> provider = TesseractOcrDataServiceProvider(logger=logger)
            >>> provider.init_provider_inputs(['./data/sample_1.png.hocr'])
            >>> provider.get_page_bbox_dict()
            [{'bbox': [0, 0, 4185, 4121], 'page': 1}]
        """
        page_bbox_dict_list = []
        for ocr_file in self._ocr_file_data_list:
            soup = bs4.BeautifulSoup(ocr_file, 'lxml')
