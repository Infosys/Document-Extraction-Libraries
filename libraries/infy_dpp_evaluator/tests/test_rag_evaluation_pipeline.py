...
def pre_test(create_root_folders, copy_files_to_root_folder):
    ...
    FILES_TO_COPY = [
        ...
        ['pipeline_rag_evaluation.json', f"{SAMPLE_ROOT_PATH}/config",
    ...
    logging_config_data = infy_fs_utils.data.LoggingConfigData(
        **{
            ...
            "log_file_data": {
                ...
                "log_file_name_prefix": "infy_dpp_evaluator",
...
def test_rag_evaluation():
    """
        Test case for testing complete RAG Evaluation pipeline
    """
    PROCESSOR_INPUT_CONFIG_PATH = '/data/config/pipeline_rag_evaluation.json'
    ...
    assert response_data_list[0].context_data.get(
        'rag_evaluator') is not None
    assert response_data_list[0].context_data.get(
        'rag_report_generator') is not None
    assert response_data_list[0].context_data.get(
