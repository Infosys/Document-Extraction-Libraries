...
def pre_test(create_root_folders, copy_files_to_root_folder):
    ...
    FILES_TO_COPY = [
        ...
        ['pipeline_qna_truth_data_eval_input_config.json', f"{SAMPLE_ROOT_PATH}/config",
            f"{STORAGE_ROOT_PATH}/data/config"],
        ['openai_count_prompt.txt', f"{SAMPLE_ROOT_PATH}/config/prompt_templates",
    ...
    logging_config_data = infy_fs_utils.data.LoggingConfigData(
        **{
            ...
            "log_file_data": {
                ...
                "log_file_name_prefix": "content_evaluation",
...
def test_qna_evaluation_1():
    """
        Test case for evaluation
    """
    start_time = time.time()
    PROCESSOR_INPUT_CONFIG_PATH = '/data/config/pipeline_qna_truth_data_eval_input_config.json'
    ...
    print(
        f"Total time taken : {elapsed_time} sec")

    assert response_data_list[0].context_data.get(
        'request_creator') is not None
    assert response_data_list[0].context_data.get(
        'content_evaluator') is not None
    assert response_data_list[0].context_data.get(
        'content_reporter') is not None
    assert response_data_list[0].context_data.get(
