...
def align_1d(sequence1, sequence2, reward_lookup, return_alignment=False):
    ...
    for seq1_idx in range(1, sequence1_length+1):
        for seq2_idx in range(1, sequence2_length+1):
            reward = reward_lookup[sequence1[seq1_idx -
                                             1] + sequence2[seq2_idx-1]]
...
def factored_2dmss(true_cell_grid, pred_cell_grid, reward_function):
    ...
    for trow, tcol, prow, pcol in itertools.product(range(true_cell_grid.shape[0]),
                                                    range(
                                                        true_cell_grid.shape[1]),
                                                    range(
                                                        pred_cell_grid.shape[0]),
...
def cells_to_relspan_grid(cells):
    ...
    for i in range(num_rows):
        for j in range(num_columns):
            # Default placeholder for empty cells
            cell_grid[i][j] = [0, 0, 0, 0]
    # print('empty cell_grid initialization:',cell_grid)
    ...
    max_length = max(len(row) for row in cell_grid)
    padded_grid = [row + [[0, 0, 0, 0]] *
                   (max_length - len(row)) for row in cell_grid]
    # return np.array(padded_grid, dtype=object)
...
def output_to_dilatedbbox_grid(bboxes, labels, scores):
    ...
    *dilated*, which means they are larger than the actual ground truth boxes.
    ...
    spanning_cells = [{'bbox': bbox, 'score': 1}
                      for bbox, label in zip(bboxes, labels) if label in [4, 5]]
    ...
    while len(stack) > 0:
        ...
        if current.tag == 'td' or current.tag == 'th':
            ...
            try:
                ...
                current_column = min(set(
                    range(max_occupied_column+2)).difference(occupied_columns_by_row[current_row]))
            except:
                current_column = 0
        ...
        for child in children[::-1]:
            stack.append((child, in_header or current.tag ==
                         'th' or current.tag == 'thead'))
...
def grits_from_html(true_html, pred_html):
    ...
    # print(true_topology_grid)
    pred_topology_grid = np.array(cells_to_relspan_grid(pred_cells))
    # print(pred_topology_grid)
    true_text_grid = np.array(cells_to_grid(
        true_cells, key='cell_text'), dtype=object)
    # print(true_text_grid)
    pred_text_grid = np.array(cells_to_grid(
        pred_cells, key='cell_text'), dtype=object)
    # print(pred_text_grid)
