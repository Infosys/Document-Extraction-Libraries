...
STORAGE_ROOT_PATH = f"C:/temp/unittest/infy_dpp_content_extractor/{__name__}/STORAGE"
CONTAINER_ROOT_PATH = f"C:/temp/unittest/infy_dpp_content_extractor/{__name__}/CONTAINER"
...
def pre_test(create_root_folders, copy_files_to_root_folder):
    ...
    FILES_TO_COPY = [
        ...
        ['pipeline_separate_table_node_input_config_data.json', f"{SAMPLE_ROOT_PATH}/config",
...
def test_dpp_extractor():
    """
        Test case for two separate nodes for (text ,image) and table extraction in sequence.
    """
    # TF pipline separate table node in sequence to content_extractor's (text & image) node
    PROCESSOR_INPUT_CONFIG_PATH = '/data/config/pipeline_separate_table_node_input_config_data.json'
    ...
    assert len(response_data_list[0].context_data) >= 3
    assert response_data_list[0].context_data['content_extractor_text'] is not None
    assert response_data_list[0].context_data['content_extractor_image'] is not None
    assert response_data_list[0].context_data['content_extractor_table'] is not None
