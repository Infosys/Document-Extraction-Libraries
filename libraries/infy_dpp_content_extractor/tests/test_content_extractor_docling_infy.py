...
def pre_test(create_root_folders, copy_files_to_root_folder):
    ...
    FILES_TO_COPY = [
        # ['survey.tiff', f"{SAMPLE_ROOT_PATH}/input",
        #     f"{STORAGE_ROOT_PATH}/data/input"],
        ['sports_statistics.pdf', f"{SAMPLE_ROOT_PATH}/input",
            f"{STORAGE_ROOT_PATH}/data/input"],
        ['pipeline_docling_input_config_data.json', f"{SAMPLE_ROOT_PATH}/config",
...
def test_dpp_extractor():
    """
        Test case for dpp_extractor
    """
    PROCESSOR_INPUT_CONFIG_PATH = '/data/config/pipeline_docling_input_config_data.json'
    ...
    assert not response_data.message_data
    assert len(response_data.context_data) >= 3
    assert response_data.context_data['content_extractor'] is not None
    docling_file_path_list = response_data.context_data['content_extractor'][
        'content_extractor_table']['docling_file_path_list']
    assert len(docling_file_path_list) == 2
