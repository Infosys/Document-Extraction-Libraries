...
class RACARProvider(IMetricsProvider):
    """Sample Metrics provider class for demonstrating a metrics calculation using LLM """
    __RELEVANCE_PROMPT_FILE_PATH = "/prompt_templates/racar/relevance_prompt.txt"
    __AGNOSTICISM_PROMPT_FILE_PATH = "/prompt_templates/racar/agnosticism_prompt.txt"
    __COMPLETESS_PROMPT_FILE_PATH = "/prompt_templates/racar/completeness_prompt.txt"
    __ACCURACY_PROMPT_FILE_PATH = "/prompt_templates/racar/accuracy_prompt.txt"
    __REASONABLENESS_PROMPT_FILE_PATH = "/prompt_templates/racar/reasonableness_prompt.txt"
    ...
    def __init__(self, llm_config_data_list: List[LLMConfigData]) -> None:
        ...
        metric_prompts = {
            "relevance": self.__get_file_content(
                f'{app_config_folder_path}/{self.__RELEVANCE_PROMPT_FILE_PATH}'),
            "agnosticism": self.__get_file_content(
                f'{app_config_folder_path}/{self.__AGNOSTICISM_PROMPT_FILE_PATH}'),
            "completeness": self.__get_file_content(
                f'{app_config_folder_path}/{self.__COMPLETESS_PROMPT_FILE_PATH}'),
            "accuracy": self.__get_file_content(
                f'{app_config_folder_path}/{self.__ACCURACY_PROMPT_FILE_PATH}'),
            "reasonableness": self.__get_file_content(
                f'{app_config_folder_path}/{self.__REASONABLENESS_PROMPT_FILE_PATH}'),
    ...
    def __llm_call_and_parse(self, content_evaluator_req_data, prompt_template,
        ...
        # Parse the score from LLM response (assuming it returns a number 1-3)
        if llm_parser_obj:
