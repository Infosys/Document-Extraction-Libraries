...
class Bm25sSparseDbProvider(ISparseDbProvider):
    ...
    def save_record(self, sparse_record_config_dict: SparseDbRecordConfigData):
        try:
            ...
            content_file_path = sparse_record_config_dict.get(
                'content_file_path', '')
            ...
            retriever = {}
            local_db_folder_path = self.__fs_handler.get_abs_path(
                db_folder_path).replace('filefile://', '')
            ...
            if os.path.exists(local_db_folder_path) and os.path.exists(f'{local_db_folder_path}/{db_index_name}/corpus.jsonl'):
                ...
                retriever = bm25s_service_obj.add_record(
                    retriever, nltk_data_dir, chunk_list, corpus)
            else:
                retriever = bm25s_service_obj.create_new(
                    nltk_data_dir, chunk_list, corpus)
    ...
    def get_matches(self, query_params_data: SparseDbQueryParamsData):
        try:
            ...
            pre_filter_fetch_k = query_params_data.get(
                'pre_filter_fetch_k', top_k*top_k)
            local_db_folder_path = self.__fs_handler.get_abs_path(
                db_folder_path).replace('filefile://', '')
            ...
            if os.path.exists(local_db_folder_path) and os.path.exists(f'{local_db_folder_path}/{db_index_name}/corpus.jsonl'):
                ...
                retriever = bm25s_service_obj.load_local()
                records = bm25s_service_obj.search_records(
                    retriever, nltk_data_dir, query, top_k, pre_filter_fetch_k)
                record_list = []
                if filter_metadata:
                    for i in range(len(records[0][0])):
                        record_metadata = records[0][0][i].get('metadata', {})
                        custom_metadata = record_metadata.get('custom_metadata', {})
                        match = True
                        for key, value in filter_metadata.items():
                            key_parts = key.split('.')
                            if len(key_parts) == 2 and key_parts[0] == 'custom_metadata':
                                if custom_metadata.get(key_parts[1]) != value:
                                    match = False
                                    break
                            else:
