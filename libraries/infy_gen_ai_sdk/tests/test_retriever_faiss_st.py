...
def test_1():
    ...
    # Query combination 1 => No filter applied
    ...
    assert set([x.metadata['company'] for x in records]
               ).issubset({'nvidia', 'ibm', 'google'})
    print(records)

    # Query combination 2 => metadata filter applied
    query_params_data = infy_gen_ai_sdk.vectordb.provider.faiss.VectorDbQueryParamsData(
        **{
            'query': "What is the amount?",
            'top_k': 1,
            'pre_filter_fetch_k': 10,
            'filter_metadata': {'company': 'nvidia'}
        })
    records: List[infy_gen_ai_sdk.vectordb.provider.faiss.MatchingVectorDbRecordData] = vector_db_provider.get_matches(
        query_params_data)
    assert len(records) == 1
    assert [x.metadata['company'] for x in records] == ['nvidia']
    print(records)

    # Query combination 3 => custom metadata filter applied
    query_params_data = infy_gen_ai_sdk.vectordb.provider.faiss.VectorDbQueryParamsData(
        **{
            'query': "What is the amount?",
            'top_k': 1,
            'pre_filter_fetch_k': 10,
            'filter_metadata': {'custom_metadata.company_type': 'technology'}
        })
    records: List[infy_gen_ai_sdk.vectordb.provider.faiss.MatchingVectorDbRecordData] = vector_db_provider.get_matches(
        query_params_data)
    assert len(records) == 1
    ...
    # Query combination 4 => custom metadata filter + general metadata filter applied
    query_params_data = infy_gen_ai_sdk.vectordb.provider.faiss.VectorDbQueryParamsData(
        **{
            'query': "What is the amount?",
            'top_k': 1,
            'pre_filter_fetch_k': 10,
            'filter_metadata': {'company': 'nvidia','custom_metadata.company_type': 'technology'}
        })
    records: List[infy_gen_ai_sdk.vectordb.provider.faiss.MatchingVectorDbRecordData] = vector_db_provider.get_matches(
        query_params_data)
    assert len(records) == 1
