...
from tests.internal import test_pt_01_indexer_faiss_openai_doc_level
...
def pre_test(create_root_folders, copy_files_to_root_folder):
    ...
    SAMPLE_ROOT_PATH = test_pt_01_indexer_faiss_openai_doc_level.STORAGE_ROOT_PATH
    FILES_TO_COPY = [
...
def test_1():
    ...
    for x in range(1, 21):
        faiss_file_path = STORAGE_ROOT_PATH + \
            '/vectordb/openai-text-davinci-003/sow_pt_doc_level_db/' + \
            str(x)
        ...
        counter += 1
        print(
            f"[End] - Document - {x}; record matched {len(records)} execution elapse time is {end_time_in_mins} mins")
