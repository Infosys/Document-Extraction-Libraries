...
PROCESSOR_INPUT_CONFIG_PATH = "/data/config/local_retriever_reader_input_config.json"
...
def pre_test(create_root_folders, copy_files_to_root_folder):
    ...
    FILES_TO_COPY = [
        ['local_retriever_reader_input_config.json', f"{SAMPLE_ROOT_PATH}/data/config",
            f"{STORAGE_ROOT_PATH}/data/config"],
        ['local_mde_retriever_reader_input_config.json', f"{SAMPLE_ROOT_PATH}/data/config",
            f"{STORAGE_ROOT_PATH}/data/config"],
        ['*.*', f"{SAMPLE_ROOT_PATH}/data/config/prompt_templates",
...
def test_dpp_local_retriever_reader():
    """
...
def test_dpp_local_retriever_reader_metadata():
    """
        Test case for ai reader pipeline
    """
    PROCESSOR_INPUT_CONFIG_PATH_metadata = "/data/config/local_mde_retriever_reader_input_config.json"
    # ---- Create response data -----
    metadata = infy_dpp_sdk.data.MetaData(
        standard_data=infy_dpp_sdk.data.StandardData(
            filepath=infy_dpp_sdk.data.ValueData()))
    ...
    # --------- Run the pipeline ------------
    dpp_orchestrator = infy_dpp_sdk.orchestrator.OrchestratorNativeBasic(
        input_config_file_path=PROCESSOR_INPUT_CONFIG_PATH_metadata)
    response_data_list = dpp_orchestrator.run_batch(
        [infy_dpp_sdk.data.DocumentData(
            **document_data_json.get('document_data'))],
        [document_data_json.get('context_data')])
