...
def pre_test(create_root_folders, copy_files_to_root_folder):
    ...
    FILES_TO_COPY = [
        ['local_indexer_input_config.json', f"{SAMPLE_ROOT_PATH}/data/config",
            f"{STORAGE_ROOT_PATH}/data/config"],
        ['local_mde_indexer_input_config.json', f"{SAMPLE_ROOT_PATH}/data/config",
            f"{STORAGE_ROOT_PATH}/data/config"],
...
def test_dpp_local_index_creation():
    """
        Test case for dpp_ai
    """
    PROCESSOR_INPUT_CONFIG_PATH = '/data/config/local_indexer_input_config.json'
...
def test_dpp_local_index_creation_using_metadata_extractor_doc():
    """
        Test case for dpp_ai
    """
    PROCESSOR_INPUT_CONFIG_PATH = '/data/config/local_mde_indexer_input_config.json'
    file_sys_handler = infy_fs_utils.manager.FileSystemManager(
    ).get_fs_handler(infy_dpp_sdk.common.Constants.FSH_DPP)
    processor_response_data = json.loads(file_sys_handler.read_file
                                         ("/data/work/D-5a449ddc-2b56-4766-b346-1f9f1e6cd31f/page-14-17.pdf_files/processor_response_data.json"))
