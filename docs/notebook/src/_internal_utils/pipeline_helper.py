...
class PipelineHelper():
    ...
    def __init__(self, pipeline_input_config_file_path, storage_root_path, container_root_path):
        self.__pipeline_input_config_file_path = storage_root_path + \
            pipeline_input_config_file_path
        self.__pipeline_file_name = os.path.basename(
            self.__pipeline_input_config_file_path)
...
#             })
    ...
    def __get_files(self, storage_root_path, sub_folder_path, search_pattern):
        ...
        files = [x.replace("//", "/") for x in files]
        files = [x.replace(storage_root_path + sub_folder_path + "/", "")
                 for x in files]
        return files
        ...
        if topic == 'RequestCreator':
            ...
            if input_folder_path:
                files = self.__get_files(
                    storage_root_path, input_folder_path, "*.*")
                ...
                files = self.__get_files(
                    storage_root_path, existing_request_file_read_path, "*.*")
                file_count = len(files)
                if file_count == 0:
                    description += f"1. No expected **group request file** found in `{existing_request_file_read_path}` folder.  "
                    description += f"\n<b><span style='color:red'>Status: Failed</span></b>  "
                    files = self.__get_files(
                        storage_root_path, existing_request_file_save_path, "*.*")
                    file_count = len(files)
                    if file_count > 0:
                    ...
                    files = self.__get_files(
                        storage_root_path, save_request_file_path, "*.*")
                    file_count = len(files)
                    if file_count > 0:
                        description += f"\n<i><span style='color:blue'>"
                        description += f"**Hint:** Found `{file_count}` **group request file(s)** in "
                    ...
                    description += f"\nFile details: `{files}`   "
                    description += f"\n<b><span style='color:green'>Status: Passed</span></b>"
                    group_queue_folder_names = self.__get_files(
                        storage_root_path, queue_folder_path, "*")
                    matches_found_list = []
                    for group_request_file in files:
                        group_request_id = group_request_file.split(
                            '_group_request.json')[0]
                        if group_request_id in group_queue_folder_names:
                            matches_found_list.append(
                                queue_folder_path + "/" + group_request_id)
