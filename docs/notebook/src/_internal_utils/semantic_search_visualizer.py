...
class SemanticSearchVisualizer():

    __COUNTS_DEFAULT_VALUE = "char(s): 0 | word(s): 0 | token(s): 0"

    def __init__(self):
        form_label_input = widgets.HTML(
            '<span style="background-color: lightgreen">Search:</span>')
        form_text_area = widgets.Textarea(rows=3, layout={'width': '95%'})
        ...
        # form_grid = widgets.GridspecLayout(2, 10)
        # form_grid[0, 0:10] = form_input_form

        form_label_output = widgets.HTML(
            '\n\n<span style="background-color: lightgreen">Results:</span>')
...
#         form_output_rhs = widgets.Label(layout={"border": "1px solid red", "padding" : "1px"})
        form_output = widgets.VBox(
            [form_label_output, form_output_rhs])

        # form_grid[1, 0:10] = form_output

        form_grid = widgets.VBox([form_input_form, form_output])
