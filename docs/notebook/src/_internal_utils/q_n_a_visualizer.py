...
class QnAVisualizer():
    ...
    def __init__(self, lhs_title='Question:', rhs_title='Answer:', show_filter=True):
        form_label_input = widgets.HTML(
                    f'<span style="background-color: lightgreen">{lhs_title}</span>')
        form_text_area = widgets.Textarea(rows=15, layout={'width': '95%'})
        if show_filter:
            form_label_filter = widgets.HTML(
                        '<span style="background-color: lightgreen">Filter:</span>')
            form_text_area_filter = widgets.Textarea(rows=1, layout={'width': '95%'})
        ...
        items_list = [form_label_input, form_text_area_info, form_text_area]
        if show_filter:
            items_list.extend([form_label_filter, form_text_area_filter])
        items_list.append(widgets.HBox([form_submit_button,form_reset_button]))
        form_input_form = widgets.VBox(items_list)

        form_grid = widgets.GridspecLayout(1, 10)
        form_grid[0, 0:4] = form_input_form

        form_label_output = widgets.HTML(
                    f'\n\n<span style="background-color: lightgreen">{rhs_title}</span>')
...
#         form_output_rhs = widgets.Label(layout={"border": "1px solid red", "padding" : "1px"})
        ...
        if show_filter:
            self.__form_text_area_filter=form_text_area_filter
        self.__output_text = ''
    ...
    def get_filter_text(self):
        form_text_area_filter = self.__form_text_area_filter
        return form_text_area_filter.value
