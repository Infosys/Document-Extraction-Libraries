...
class FileSystemHandler:
    ...
    def move_directory(self, source_path, target_path):
        """Moves a directory from `source_path` to `target_path`."""
        try:
            # List all files in the source directory
            files = self.__fs.ls(self.__prepare_uri(source_path), detail=False)

            # Move each file to the target directory
            for file in files:
                self.__fs.mv(
                    file, f"{self.__prepare_uri(target_path)}/{file.split('/')[-1]}")

            # Delete the source directory
            self.__fs.rm(self.__prepare_uri(source_path), recursive=True)
