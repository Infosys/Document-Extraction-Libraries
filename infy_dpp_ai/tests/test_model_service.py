...
def pre_test(create_root_folders, copy_files_to_root_folder,backup_folder,restore_folder):
    ...
    restore_folder(f"{BACKUP_ROOT_PATH}/cache",f"{STORAGE_ROOT_PATH}/data/cache")   
...
def test_model_service():
    ...
    input_config_data['processor_input_config']['QueryRetriever']['queries']=[query_dict]
    ...
    file_sys_handler.write_file(PROCESSOR_INPUT_CONFIG_PATH,json.dumps(input_config_data,indent=4))
...
def test_model_service_prompt():
    ...
    for llm_name,llm_detail in input_config_data['processor_input_config']['Reader']['llm'].items():        
                    if llm_detail.get('enabled'):
                        ...
                        if cache:
                            cache['enabled']=cache_enabled
    prompt_template=input_config_data['processor_input_config']['Reader']["inputs"][0].get("prompt_template")  
    existing_content_list=input_config_data['processor_input_config']['Reader']["named_prompt_templates"].get(prompt_template)['content']
    input_config_data['processor_input_config']['Reader']["named_prompt_templates"].get(prompt_template)['content']=content
    file_sys_handler.write_file(PROCESSOR_INPUT_CONFIG_PATH,json.dumps(input_config_data,indent=4))
    print(input_config_data)
    ...
    input_config_data['processor_input_config']['Reader']["named_prompt_templates"].get(prompt_template)['content']=existing_content_list
    file_sys_handler.write_file(PROCESSOR_INPUT_CONFIG_PATH,json.dumps(input_config_data,indent=4))
    # --------- Save the response data to temp file ------------
    for response_data in response_data_list:        
        output_file_path="/data/work/D-037c85b3-d45b-47db-abb0-b7aadf813b4e/page-14-17.pdf_files/processor_response_data.json"
