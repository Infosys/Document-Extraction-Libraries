...
class PipfileUtil:
    ...
    def extract_pkg_versions(pipfile_path):
        ...
        for line in nonempty_line_list:
            if line.strip() == "[packages]":
                copy = True
                continue
            elif line.strip() == "[requires]" or line.strip().startswith("#[extra-"):
                copy = False
                continue
            elif copy:
    ...
    def extract_extras_versions(pipfile_path):
        ...
        extras_name = None
        for line in nonempty_line_list:
            if line.strip().startswith("#[extra-"):  # E.g. #[extra-tesseract]
                copy = True
        ...
        for extras_name in extras_name_list:
            ...
            extras_dict[extras_name] = [x.replace('=', '', 1).replace('"', '') if 'win32' not in x
                                        else x.replace(" ", "").replace('{version=', '')
                                        .replace('}', '').replace("=", "", 1)
                                        .replace(',sys_platform=', ' ;sys_platform')
                                        .replace('"', '')
