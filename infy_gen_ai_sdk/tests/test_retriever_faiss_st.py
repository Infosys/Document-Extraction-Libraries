...
def test_1():
    ...
    # Query combination 1 => top_k < pre_filter_fetch_k
    ...
    assert [x.metadata['company']
            for x in records] == ['nvidia', 'ibm', 'google']
    print(records)

    # Query combination 2 => top_k = 1
    query_params_data = infy_gen_ai_sdk.vectordb.provider.faiss.VectorDbQueryParamsData(
        **{
            'query': "What is the amount?",
            'top_k': 1,
    ...
    # Query combination 3 => top_k > pre_filter_fetch_k
    query_params_data = infy_gen_ai_sdk.vectordb.provider.faiss.VectorDbQueryParamsData(
        **{
            'query': "What is the amount?",
            'top_k': 10,
            'pre_filter_fetch_k': 1,
            'filter_metadata': {
            }
        })
    records: List[infy_gen_ai_sdk.vectordb.provider.faiss.MatchingVectorDbRecordData] = vector_db_provider.get_matches(
    ...
    # Query combination 4 => metadata filter applied
    query_params_data = infy_gen_ai_sdk.vectordb.provider.faiss.VectorDbQueryParamsData(
        **{
            'query': "What is the amount?",
            'top_k': 5,
    ...
    records: List[infy_gen_ai_sdk.vectordb.provider.faiss.MatchingVectorDbRecordData] = vector_db_provider.get_matches(
        query_params_data)
    assert len(records) == 1
    assert [x.metadata['company'] for x in records] == ['google']
    print(records)
